...
(function() {
    ...
    let GATTServer; // 全局GATTServer
    let sendCount = 0; // 发送计数
...
`;
    ...
    const scanBluetooth = () => {
        if (typeof navigator.bluetooth === 'undefined') {
            createWindow('您的浏览器不支持蓝牙API，请更换为Chrome浏览器', 'OK');
            return;
        }
        if (GATTServer) {
            createWindow('已经连接蓝牙设备', 'OK');
            return;
        }

        createWindow('正在搜索蓝牙设备...', '', 'scanBluetoothWindow');
        ...
        navigator.bluetooth.requestDevice({
            ...
            createWindow('成功连接到 ' + device.name + '，正在获取服务...', '', 'scanBluetoothWindow');
            ...
            document.getElementById('scanBluetoothWindow').remove();
            switch (error.code) {
                case 8: createWindow('已取消蓝牙扫描', 'OK'); break;
                case 18: createWindow('由于浏览器安全限制，请在多邻国网页内完成任意操作后再次尝试连接', 'OK'); break;
                default: createWindow('出现异常(' + error.code + ')：' + error, 'OK');
            }
            console.error(error.code);
            //createWindow('异常：连接设备失败。error: ' + error.code, 'OK');
        });
    }
    ...
    const hexStringToUint8Array = (hexString) => {
        if (hexString.length % 2 !== 0) {
            createWindow('Hex string length must be even', 'OK');
